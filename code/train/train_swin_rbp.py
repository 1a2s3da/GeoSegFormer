"""Train RBP-UNet with Swin-T backbone."""
...
from models.rbp_unet_swin import RBPUNetSwin
...
def main():
    ...
    ap.add_argument('--lambda_b', type=float, default=0.2)
    ap.add_argument('--lambda_c', type=float, default=0.05)
    ap.add_argument('--lambda_p', type=float, default=0.05)
    ap.add_argument('--proto_warmup', type=int, default=10)
    ap.add_argument('--variant', type=str, default='swin_t', choices=['swin_t', 'swin_b'])
    ap.add_argument('--out_dir', type=str, default=os.path.join(_ROOT, 'runs', 'rbp_swin'))
    ap.add_argument('--data_root', type=str,
                    default=os.path.join(_ROOT, 'data', 'plantsegv3'))
    ap.add_argument('--pretrained', type=str,
                    default=os.path.join(_ROOT, 'pretrained', 'swin_t.pth'))
    args = ap.parse_args()
    ...
    print(f'train: {len(tr_ds)}, val: {len(va_ds)}, img_size={args.img_size}')

    model = RBPUNetSwin(num_classes=args.num_classes, variant=args.variant,
    ...
    def poly_lr(step):
        return (1 - step / n_steps) ** args.poly_power
    ...
    for ep in range(args.epochs):
        ...
        print(f'Ep {ep+1:3d}/{args.epochs} | total={tl["total"]:.4f} '
              ...
              f'lr={cur_lr:.5f} '
              f'| val mIoU={res["mIoU"]:.4f} mAcc={res["mAcc"]:.4f} | {elapsed:.1f}s', flush=True)
